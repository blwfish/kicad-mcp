            ...
            for ic_ref, ic_component in components.items():
                if ic_ref.startswith("U") or ic_ref.startswith("IC"):
                    ...
                    for converter_type, pattern in switching_patterns.items():
                        if re.search(
                            pattern, ic_value, re.IGNORECASE
                        ) or re.search(pattern, ic_lib, re.IGNORECASE):
                            power_supplies.append({
...
def identify_amplifiers(
        ...
        for pattern in opamp_patterns:
            if re.search(pattern, component_value, re.IGNORECASE) or re.search(
                ...
                # Instrumentation amplifiers
                    ...
                    r"INA\d{3}|AD620|AD8221|AD8429|LT1167",
                    component_value,
                    re.IGNORECASE,
                ):
                    amplifiers.append({
    ...
    for ref in transistor_refs:
        ...
        if "BJT" in component_lib or "NPN" in component_lib or "PNP" in component_lib:
            ...
            "FET" in component_lib
            or "MOSFET" in component_lib
            or "JFET" in component_lib
        ):
            has_biasing = False
            for net_name, pins in nets.items():
                if any(pin.get("component") == ref for pin in pins):
    ...
    for op_ref in opamp_refs:
        ...
        for net_name, pins in nets.items():
            if any(pin.get("component") == op_ref for pin in pins):
                connects_to_r = any(
                    pin.get("component", "").startswith("R") for pin in pins
                )
                connects_to_c = any(
                    pin.get("component", "").startswith("C") for pin in pins
                )
    ...
    for ref, component in components.items():
        ...
        for family, pattern in mcu_patterns.items():
            if re.search(pattern, component_value, re.IGNORECASE) or re.search(
                ...
                if re.search(
                    r"ATMEGA328P|ATMEGA328", component_value, re.IGNORECASE
                ):
                    ...
                    model = re.search(
                        r"(PIC\d+\w+)", component_value, re.IGNORECASE
                    )
                    if model:
                        microcontrollers.append({
                            "type": "microcontroller",
                    ...
                    model = re.search(
                        r"(MSP430\w+)", component_value, re.IGNORECASE
                    )
                    if model:
                        microcontrollers.append({
                            "type": "microcontroller",
