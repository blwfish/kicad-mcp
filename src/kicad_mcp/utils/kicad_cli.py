...
class KiCadCLIManager:
    ...
    def find_kicad_cli(self, force_refresh: bool = False) -> str | None:
        ...
        if cli_path:
            if self._validate_cli_path(cli_path):
                ...
                logger.info("Found KiCad CLI at: %s", cli_path)
                return cli_path
            else:
                logger.warning("Found KiCad CLI at %s but validation failed", cli_path)
    ...
    def _detect_cli_path(self) -> str | None:
        ...
        if env_path and os.path.isfile(env_path) and os.access(env_path, os.X_OK):
            logger.info("Using KiCad CLI from environment: %s", env_path)
            return env_path
        ...
        if system_path:
            logger.info("Found KiCad CLI in system PATH: %s", system_path)
            return system_path
