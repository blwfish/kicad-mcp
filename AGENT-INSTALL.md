...
**Model-level testing (2026-09-20):** critical-rule adherence and tool-schema usability have been smoke-tested against three local, non-Claude models — `qwen/qwen2.5-coder-14b`, `qwen/qwen3-32b`, and `google/gemma-4-e4b`, all via LM Studio — using a script that speaks this server's real MCP protocol directly, not through LM Studio's own MCP handling. Two separate gaps make LM Studio's own MCP support impractical to script today: its CLI (`lms chat`) is built on the OpenAI-compatible `/v1/chat/completions` endpoint, which has no way to reference an `mcp.json`-configured server at all; and its newer native endpoint (`/api/v1/chat`) does accept an `integrations: [{"type": "plugin", "id": "<name>"}]` parameter that can invoke one, but a fresh local install returns `"Permission denied to use plugin '<name>'"` for it by default — confirmed empirically, not just from docs, including finding and correcting the parameter's real field name (`id`, not the `plugin_id` the docs' own example showed for a different integration type). That permission looks GUI-granted, with no CLI or `settings.json` equivalent found. Practically, MCP-driven local-model chat is a GUI-only feature today; a script wanting to drive one must bring its own MCP client, as this repo's probe does. Across every model tested, including the smallest (4B), the model correctly reached for `autoroute(operation="run")` over manual routing and `library(operation="search")` over guessing a symbol/footprint name, with the server's `instructions` delivered as a system prompt. `get_usage_guidance()` was not spontaneously called by any model in these tests — the in-response escalation notes on `audit(operation="placement")` and `pcb(operation="add_trace"/"add_via")` are the more load-bearing channel for surfacing guidance to a model that hasn't read the docs. This testing also caught and fixed two real bugs: `bulk_assign_pad_nets` crashing (instead of returning a clean error) on a malformed assignment dict, and every `pcbnew.LoadBoard()` call site failing to check for KiCad's `None`-on-failure return value.
